...
public class Forwarding extends ForwardingBase implements IFloodlightModule, IOFSwitchListener {
	...
	public Command processPacketInMessage(IOFSwitch sw, OFPacketIn pi, IRoutingDecision decision, FloodlightContext cntx) {
		...
		if (decision != null) {
			if (log.isTraceEnabled()) {
				log.trace("Forwarding decision={} was made for PacketIn={}, eth={}", new Object[]{decision.getRoutingAction().toString(), pi, eth});
			}
			...
			if (log.isTraceEnabled()) {
				log.trace("No decision was made for PacketIn={}, eth={}, forwarding", pi, eth);
			}
	...
	protected Match createMatchFromPacket(IOFSwitch sw, OFPort inPort, FloodlightContext cntx) {
		...
		boolean ipv4Flow = (EthType.IPv4.equals(eth.getEtherType()));
		// add in port to match options
		if(!ipv4Flow || FLOWMOD_DEFAULT_MATCH_IN_PORT) {
			mb.setExact(MatchField.IN_PORT, inPort);
		}

		if (!ipv4Flow || FLOWMOD_DEFAULT_MATCH_MAC) {
		...
		if (eth.getEtherType() == EthType.IPv4) { /* shallow check for equality is okay for EthType */
			...
			if (FLOWMOD_DEFAULT_MATCH_IP_ADDR) {
				...
				// parse the newly added another option (ipDstOnly)
				if(!FLOWMOD_DEFAULT_MATCH_IP_DST_ONLY) {
	...
	public void init(FloodlightModuleContext context) throws FloodlightModuleException {
		...
		if (tmp != null) {
			tmp = tmp.toLowerCase();
			// customize to add another option
			if (!tmp.contains("vlan") && !tmp.contains("mac") && !tmp.contains("ip") && !tmp.contains("port") &&
					!tmp.contains("ipDstOnly") && !tmp.contains("inPort")) {
				...
				FLOWMOD_DEFAULT_MATCH_IP_DST_ONLY = tmp.contains("ipDstOnly".toLowerCase()) ? true : false;
				FLOWMOD_DEFAULT_MATCH_IN_PORT = tmp.contains("inPort".toLowerCase()) ? true : false;

			}
		}
		log.info("Default flow matches set to: INPORT=" + FLOWMOD_DEFAULT_MATCH_IN_PORT
				+ ", VLAN=" + FLOWMOD_DEFAULT_MATCH_VLAN
				+ ", MAC=" + FLOWMOD_DEFAULT_MATCH_MAC
				+ ", IP=" + FLOWMOD_DEFAULT_MATCH_IP_ADDR
				+ ", IP_DST_ONLY=" + FLOWMOD_DEFAULT_MATCH_IP_DST_ONLY
